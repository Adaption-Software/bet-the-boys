...
const classes = computed(() =>
    props.active
        ? 'block w-full ps-3 pe-4 py-2 border-l-4  text-start text-base font-medium text-tertiary-300 bg-tertiary-50 bg-tertiary-800/50 focus:outline-none focus:text-tertiary-200 focus:bg-tertiary-900 focus:border-tertiary-300 transition duration-150 ease-in-out'
...
<template>
    <Link :href="href" :class="classes">
        <FontAwesome v-if="icon" :icon="props.icon" class="mr-3" />
        <slot />
    </Link>
</template>
