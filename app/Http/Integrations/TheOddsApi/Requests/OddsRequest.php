    ...
    public function createDtoFromResponse(Response $response): mixed
    {
        return $response->collect()
            ->map(function ($event) {
                ...
                $spreads = collect($markets->get('spreads'))->mapWithKeys(function ($item) {
                    return [$item['name'] => [
                        'price' => $item['price'],
                        'point' => $item['point'],
                    ]];
                ...
                return [
                    ...
                    'home_team' => $this->team($home, $spreads, $under, BetType::Under),
                    'away_team' => $this->team($away, $spreads, $over, BetType::Over),
                ];
            });
    }

    protected function team($team, $spreads, $totals, $type): array
    {
        $currentSpread = $spreads->get($team);
        $point = $currentSpread['point'] ?? 0;

        $cast = $point < 0 ? BetType::Favorite : BetType::Dawg;
